...
---
-- Grant usage down the hierarchy
GRANT USAGE ON DATABASE GITHUB TO ROLE ACCOUNTADMIN;
GRANT USAGE ON SCHEMA GITHUB.RAW TO ROLE ACCOUNTADMIN;
GRANT USAGE ON SCHEMA GITHUB.STAGING TO ROLE ACCOUNTADMIN;
GRANT USAGE ON SCHEMA GITHUB.MARTS TO ROLE ACCOUNTADMIN;

-- Existing objects
GRANT SELECT ON ALL TABLES IN SCHEMA GITHUB.RAW TO ROLE ACCOUNTADMIN;
GRANT SELECT ON ALL TABLES IN SCHEMA GITHUB.STAGING TO ROLE ACCOUNTADMIN;
GRANT SELECT ON ALL TABLES IN SCHEMA GITHUB.MARTS TO ROLE ACCOUNTADMIN;
GRANT SELECT ON ALL VIEWS IN SCHEMA GITHUB.STAGING TO ROLE ACCOUNTADMIN;
GRANT SELECT ON ALL VIEWS IN SCHEMA GITHUB.MARTS TO ROLE ACCOUNTADMIN;

-- Future objects (so you don't have to re-run after each Airbyte sync / dbt run)
GRANT SELECT ON FUTURE TABLES IN SCHEMA GITHUB.RAW TO ROLE ACCOUNTADMIN;
GRANT SELECT ON FUTURE TABLES IN SCHEMA GITHUB.STAGING TO ROLE ACCOUNTADMIN;
GRANT SELECT ON FUTURE TABLES IN SCHEMA GITHUB.MARTS TO ROLE ACCOUNTADMIN;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA SCHEMA GITHUB.STAGING TO ROLE ACCOUNTADMIN;
GRANT SELECT ON FUTURE VIEWS IN SCHEMA GITHUB.MARTS TO ROLE ACCOUNTADMIN;